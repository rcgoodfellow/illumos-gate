 ...
 * means of translating to and from a known uint32_t value. Our assumption is
 * that strings do not need translation. There should be one table of values
 * which is then wrapped up inside something else.
 */
typedef struct {
	uint32_t xp_val;
	const char *xp_name;

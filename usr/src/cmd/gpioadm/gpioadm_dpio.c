...
gpioadm_dpio_list_help(const char *fmt, ...)
{
	...
	(void) fprintf(stderr, "\nList information about DPIOs\n\n"
	    ...
	    "Filters restrict output to the named DPIOs. Each filer is "
...
typedef struct gpioadm_dpio_list_ofmt {
	const char *gdlo_dpio;
	xpio_dpio_info_t *gdlo_info;
} gpioadm_dpio_list_ofmt_t;
...
{
	gpioadm_dpio_list_ofmt_t *gdlo = ofarg->ofmt_cbarg;
	xpio_dpio_info_t *info = gdlo->gdlo_info;
	dpio_flags_t flags;

	switch (ofarg->ofmt_id) {
	case GPIOADM_DPIO_LIST_DPIO:
		if (strlcpy(buf, gdlo->gdlo_dpio, buflen) >= buflen) {
			return (B_FALSE);
		}
		break;
...
gpioadm_dpio_list_cb(xpio_t *xpio, xpio_dpio_disc_t *disc, void *arg)
{
	...
	const char *name = di_minor_name(disc->xdd_minor);
	gpioadm_dpio_list_ofmt_t ofmt;
...
gpioadm_dpio_undefine_help(const char *fmt, ...)
{
	...
	(void) fprintf(stderr, "Usage:  gpioadm dpio undefine controller/gpio");

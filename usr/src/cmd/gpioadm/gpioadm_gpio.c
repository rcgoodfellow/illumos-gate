...
gpioadm_gpio_attr_set_help(const char *fmt, ...)
{
	...
	(void) fprintf(stderr, "Usage:  gpioadm gpio attr set -c controller "
	    "-g gpio attr=value [attr=value...]\n");
	(void) fprintf(stderr, "\nSets the attributes of a single GPIO."
	    "All specified attributes are\napplied at once.\n\n"
	    "\t-c controller\tspecify the GPIO controller to query\n"
	    "\t-g gpio\t\tindicate GPIO to query (by name or id)\n");
}

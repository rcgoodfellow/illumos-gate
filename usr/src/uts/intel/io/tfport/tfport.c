...
#define	TFPORTINFO		"Tofino Switch Port Multiplexer"
...
tfport_tx_one(tfport_port_t *portp, mblk_t *mp_head)
{
	...
	mac_tx(srcp->tps_mch, tx_buf, 0, MAC_DROP_ON_NO_DESC, NULL);

	// On success, the lower level is responsible for the transmit mblk.  If
	// that was our temporary mblk, then it is our responsibility to free
	// the original mblk.
	if (tx_buf != mp_head)
		freeb(mp_head);
	return (0);
...
tfport_port_fini(tfport_t *devp, tfport_port_t *portp)
{
	...
	snprintf(name, sizeof(name), "tfport%d", portp->tp_port);
...
tfport_ioc_create(void *karg, intptr_t arg, int mode, cred_t *cred, int *rvalp)
{
	...
	// If we ever want to support multiple sources, we would check a list of
	// open sources for the requested pkt_id rather than requiring that the
	// one tfp_source match the requested pkt_id.
...
tfport_find(tfport_t *devp, datalink_id_t link)
{
	...
	for (portp = list_head(&devp->tfp_source->tps_ports);
...
tfport_ioc_delete(void *karg, intptr_t arg, int mode, cred_t *cred, int *rvalp)
{
	...
	if (portp == NULL) {
		...
		if (portp->tp_run_state != TFPORT_RUNSTATE_STOPPED) {
			dev_err(devp->tfp_dip, CE_WARN, "port %d is busy", link);
			rval = EBUSY;
		} else {
...
tfport_ioc_info(void *karg, intptr_t arg, int mode, cred_t *cred, int *rvalp)
{
	...
	if (portp == NULL) {
		...
		bcopy(portp->tp_mac_addr, iarg->tii_mac_addr, iarg->tii_mac_len);
		mutex_exit(&portp->tp_mutex);
	}
	mutex_exit(&devp->tfp_mutex);
    ...
    mblk_t *mp)
{
	...
	if (addr->sa_family == AF_INET) {
		...
		dev_err(devp->tfp_dip, CE_NOTE, "ipv6 addr: %s on %d",
		    inet_ntop(AF_INET6, &sin6->sin6_addr, buf1, sizeof (buf1)), ip2m.ip2mac_ifindex);
	} else {
	...
	cnt++;
	(void) ip2mac(IP2MAC_RESOLVE, &ip2m, tfport_ioc_l2_done, (void *)cnt, 0);

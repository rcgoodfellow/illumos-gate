 ...
 * in that userland can consume. However, for the kernel all GPIOs are
 * identified by its controller and ID. The kgpio driver does not track
 * individual GPIOs.
 *
 ...
 * in data that isn't tied t the instance mostly for the reason as the core of
 ...
 *     data either in this driver or in the individual provider's. Doing
 ...
 * having the provider map that to hardware states. Provider's are allowed not
 * to know states and much fail on unknown values, allowing us to expand the set
 * of states ad we have future needs. Because the input and output values are
 * always checked to be a uint32_t, that gives us some future flexibility at
 ...
 *     is no easy way to go from a GPIO and see which GPIOs point into it. When
 ...
 *     though, when that is added, we should ensure that the provider's are
...
typedef enum {
	 ...
	 * kgpio_t has a DDI hold on it. This is established in a controllers
    ...
    uint32_t ngpio)
{
	...
	if (kgpio != NULL) {
		 ...
		 * its given us have changed, but otherwise, we don't need to
...
kgpio_ioctl_dpio_info_search(intptr_t arg, int mode)
{
	...
	for (i = 0; i < sizeof (info.dpi_dpio); i++) {
		if (info.dpi_dpio[i] == '\0')
			break;
	}

	if (i == 0 || i == sizeof (info.dpi_dpio)) {
		return (EINVAL);
	}
...
kgpio_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	 ...
	 * worry about failure.
	 */
	if (ddi_create_minor_node(dip, "dpinfo", S_IFCHR, KGPIO_MINOR_DPINFO,
	    DDI_PSEUDO, 0) != 0) {
...
kgpio_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
	...
	ddi_remove_minor_node(dip, "dpinfo");
	kgpio_g_dip = NULL;
	mutex_exit(&kgpio_g_mutex);
	return (DDI_SUCCESS);
}
